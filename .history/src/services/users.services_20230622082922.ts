...
class UsersServices {
  ...
  private signAccessAndRefreshTokenAndEmailVerifyToken(user_id: string) {
    return Promise.all([this.signAccessToken(user_id), this.signRefreshToken(user_id), this.signEmailVerifyToken])
  }
  ...
  async register(payload: RegisterReqBody) {
    const result = await databaseService.users.insertOne(
      ...
      new User({
        ...
        email_verify_token: this.signEmailVerifyToken(payload.email)
      })
    )
    ...
    const [access_token, refresh_token, email_verify_token] = await this.signAccessAndRefreshTokenAndEmailVerifyToken(user_id)
